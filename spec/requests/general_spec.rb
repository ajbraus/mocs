...
describe "Home page before sign in" do 
  visit root_path
  it "should have header" do
    page.should_not have_content("MocsforDocs")
  end
end
...
describe "Home page after sign in" do
  ...
  let(:stockholm) { FactoryGirl.create(:organization, :name => "Stockholm, Sweden", timezone: "Paris") }

  let(:user) { FactoryGirl.create(:user, :organization => organization) }
  let(:other_user) { FactoryGirl.create(:user, :organization => organization) }

  let(:stockholm_user) { FactoryGirl.create(:user, :organization => stockholm) }
  ...
  #describe "visiting stockholm" do
    # before do
    #   user.update_attributes(:organization => stockholm)
    #   visit root_path
    # end
